...
public class GeneratePartialFeatures : WorldGenPartial
{
    ...
    public override void GeneratePartial(IServerChunk[] chunks, int mainChunkX, int mainChunkZ, int generatingChunkX, int generatingChunkZ)
    {
        ...
        var ownHeightMap = chunks[0].MapChunk.WorldGenTerrainHeightMap;
        var heightMap = mapChunk.WorldGenTerrainHeightMap;
        var riverMap = mapChunk.GetModdata<ushort[]>("riverDistance");

        if (riverMap == null) return;

        var startX = generatingChunkX * chunkSize;
        var startZ = generatingChunkZ * chunkSize;

        // Get 0-255 rain.
        var climateMap = mapChunk.MapRegion.ClimateMap;
        var regionChunkSize = sapi.WorldManager.RegionSize / chunkSize;
        var cFac = (float)climateMap.InnerSize / regionChunkSize;
        var rlX = generatingChunkX % regionChunkSize;
        var rlZ = generatingChunkZ % regionChunkSize;
        ...
        var rain = (GameMath.BiLerpRgbColor(0.5f, 0.5f, climateUpLeft, climateUpRight, climateBotLeft, climateBotRight) >> 8) & 0xFF;
        var dry = rain < 100;
        ...
        {
            for (var x = 0; x < feature.tries; x++)
            {
                if (chunkRand.NextFloat() >= feature.chance) continue;

                var randX = chunkRand.NextInt(chunkSize);
                var randZ = chunkRand.NextInt(chunkSize);
                ...
                var rockId = mapChunk.TopRockIdMap[(randZ * chunkSize) + randX];
