...
public class RiverGenerator
{
    ...
    public void ValidateSegments(RiverSegment[] segments, double maxValleyWidth, double x, double z, List<RiverSegment> valid)
    {
        ...
        for (var b = 0; b < segments.Length; b++)
        {
            ...
            var riverProjection = RiverMath.GetProjection(point, segment.riverNode.startPos, segment.riverNode.endPos); // How far along the main river for size calculation.

            var riverSize = GameMath.Lerp(segment.riverNode.startSize, segment.riverNode.endSize, riverProjection); // Size of river.

            var segmentProjection = RiverMath.GetProjection(point, segment.startPos, segment.endPos);
            ...
            for (var i = 0; i < connectors.Length; i++)
            {
                ...
                var distance = RiverMath.DistanceToLine(point, lerpedStart, lerpedEnd);
                distance -= riverSize;
    ...
    public RiverSample SampleRiver(RiverSegment[] segments, double x, double z)
    {
        ...
        var closestLine = double.PositiveInfinity;
            ...
            for (var i = 0; i < connectors.Length; i++)
            {
                ...
                if (distance <= riverSize) // If within bank distance.
                {
                    ...
                    if (distance < closestLine)
                    {
                        ...
                        var segmentFlowVector = lerpedStart - lerpedEnd;
                    ...
                    var lerp = RiverMath.InverseLerp(distance, riverSize, 0);
